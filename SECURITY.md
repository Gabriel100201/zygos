...
[GitHub's private vulnerability reporting](https://github.com/Gabriel100201/tablero/security/advisories/new)
rather than opening a public issue.

Include the version (`tablero version`), the provider type involved, and the
...
Tablero runs locally, on your machine, under your user account. It has no
server, no telemetry, and no network destination other than the provider APIs
you configure yourself.

**Credentials are stored in plaintext.** `~/.tablero/config.yaml` holds Linear
...
platform. What Tablero does instead:
...
- credentials are never included in error messages, log output, or MCP
  responses — `tablero config list` masks them;
- `TABLERO_CONFIG` lets you relocate the file onto an encrypted volume.
...
- The agent's behaviour. Tablero exposes tools; deciding which of them to call
